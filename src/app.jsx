...
const D = {
  ...
  imarTipi:"MANUEL", koordLat:"", koordLng:"",
        ...
        {tab==="inputs" && (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:20 }}>
            <div>
              ...
              {(()=>{
                const IMAR_TIPLERI = [
                  { k:"KONUT",   l:"🏠 Konut",          kaks:1.50, taks:0.40, h:9.50,  nizam:"AYRK", on:5, arka:3, yan:3 },
                  { k:"KONUT_Y", l:"🏠 Konut (Yoğun)",  kaks:2.50, taks:0.50, h:15.50, nizam:"AYRK", on:5, arka:3, yan:3 },
                  { k:"TICARI",  l:"🏪 Ticari",          kaks:2.00, taks:0.50, h:12.50, nizam:"BİTİŞ", on:0, arka:0, yan:0 },
                  { k:"MIA",     l:"🏙️ MİA",             kaks:3.00, taks:0.60, h:0,     nizam:"BİTİŞ", on:0, arka:0, yan:0 },
                  { k:"IBT",     l:"🏢 İBT",             kaks:2.50, taks:0.55, h:0,     nizam:"AYRK", on:5, arka:5, yan:5 },
                  { k:"TURIZM",  l:"🏨 Turizm",          kaks:1.20, taks:0.30, h:9.50,  nizam:"AYRK", on:10, arka:5, yan:5 },
                  { k:"SANAYI",  l:"🏭 Sanayi",          kaks:1.00, taks:0.50, h:0,     nizam:"AYRK", on:10, arka:5, yan:5 },
                  { k:"TARIM",   l:"🌾 Tarım/Kırsal",   kaks:0.05, taks:0.05, h:6.50,  nizam:"AYRK", on:20, arka:10, yan:10 },
                  { k:"MANUEL",  l:"✏️ Manuel Giriş",   kaks:null,  taks:null, h:null,  nizam:null },
                ];
                const secili = IMAR_TIPLERI.find(t=>t.k===inp.imarTipi)||IMAR_TIPLERI[IMAR_TIPLERI.length-1];
                return (
                  <div style={{marginBottom:14}}>
                    <label style={{display:"block",fontSize:10,color:th.accent,marginBottom:6,letterSpacing:"0.07em",fontWeight:700}}>
                      🏛️ {lang==="TR"?"İMAR KULLANIM TÜRÜ":"ZONING TYPE"}
                    </label>
                      ...
                      {IMAR_TIPLERI.map(t=>(
                        <button key={t.k} onClick={()=>{
                          ...
                          if(t.kaks!==null){
                            ...
                            set("nizam",t.nizam);
                            set("onCekme",t.on);
                            set("arkaCekme",t.arka);
                            set("yanCekme",t.yan);
                          }
                        }}
                          style={{background:inp.imarTipi===t.k?th.accent+"25":th.bg3,border:`1px solid ${inp.imarTipi===t.k?th.accent:th.border}`,color:inp.imarTipi===t.k?th.accent:th.text2,borderRadius:7,padding:"7px 4px",cursor:"pointer",fontSize:10,fontFamily:"inherit",textAlign:"center",transition:"all 0.15s"}}>
                          {t.l}
                        </button>
                      ))}
                    </div>
                    {inp.imarTipi && inp.imarTipi!=="MANUEL" && (
                      <div style={{fontSize:10,color:th.green,background:th.green+"15",padding:"6px 10px",borderRadius:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span>✅ KAKS:{secili.kaks} TAKS:{secili.taks} {secili.h?`H:${secili.h}m`:""} {secili.nizam}</span>
                        <span style={{color:th.text3,fontSize:9}}>Manuel değiştirilebilir ↓</span>
                      </div>
                    )}
                  </div>
                );
              ...
              {/* Konum + Koordinat → Otomatik Adres */}
              <div style={{marginBottom:8}}>
                <label style={{display:"block",fontSize:10,color:th.accent,marginBottom:6,letterSpacing:"0.07em",fontWeight:700}}>
                  📍 {lang==="TR"?"KONUM":"LOCATION"}
                  <span style={{fontWeight:400,color:th.text3,marginLeft:6,fontSize:9}}>
                    {lang==="TR"?"Koordinat girerseniz otomatik doldurur":"Enter coordinates to auto-fill"}
                  </span>
                </label>
                {/* Koordinat alanları */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:6,marginBottom:8}}>
                  {[
                    {l:"Enlem (Lat)", f:"koordLat", ph:"36.8969"},
                    {l:"Boylam (Lng)", f:"koordLng", ph:"30.7133"},
                  ].map(({l,f,ph})=>(
                    <div key={f}>
                      <div style={{fontSize:9,color:th.text3,marginBottom:2}}>{l}</div>
                      <input type="text" value={inp[f]||""} onChange={e=>set(f,e.target.value)} placeholder={ph}
                        style={{background:th.input,border:`1px solid ${th.accent}40`,color:th.inputText||th.accent,borderRadius:6,padding:"6px 8px",fontSize:11,fontFamily:"monospace",width:"100%",outline:"none"}} />
                    </div>
                  ))}
                  <div style={{display:"flex",alignItems:"flex-end"}}>
                    <button onClick={async()=>{
                      const lat=inp.koordLat, lng=inp.koordLng;
                      if(!lat||!lng) return;
                      try {
                        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&accept-language=tr`);
                        const d = await res.json();
                        const a = d.address||{};
                        if(a.province||a.state) set("il", a.province||a.state||"");
                        if(a.county||a.district) set("ilce", a.county||a.district||"");
                        if(a.suburb||a.neighbourhood||a.quarter) set("mahalle", a.suburb||a.neighbourhood||a.quarter||"");
                      } catch{}
                    }}
                      style={{background:`linear-gradient(135deg,${th.accent},#e8c060)`,border:"none",color:"#070a0f",borderRadius:6,padding:"6px 10px",cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:"inherit",whiteSpace:"nowrap"}}>
                      🔍 Bul
                    </button>
                  </div>
                </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:4}}>
                {[
                  {l:t.fields.il, f:"il", tip:"Örn: Antalya"},
                  {l:t.fields.ilce, f:"ilce", tip:"Örn: Kepez"},
                ].map(p=>(
                  <div key={p.f} style={{marginBottom:8}}>
                    <label style={{display:"block",fontSize:10,color:th.text3,marginBottom:3,letterSpacing:"0.07em"}}>{p.l}</label>
                    <input type="text" value={inp[p.f]||""} onChange={e=>set(p.f,e.target.value)} placeholder="—"
                      style={{background:th.input,border:`1px solid ${th.accent}40`,color:th.inputText||th.accent,borderRadius:6,padding:"7px 10px",fontSize:12,fontFamily:"'Courier Prime',monospace",fontWeight:700,width:"100%",outline:"none"}} />
                  </div>
                ))}
              </div>
              {[
                {l:t.fields.mahalle, f:"mahalle"},
                {l:t.fields.adaNo,   f:"adaNo"},
                {l:t.fields.parselNo,f:"parselNo"},
              ].map(p=>(
                <div key={p.f} style={{marginBottom:8}}>
                  <label style={{display:"block",fontSize:10,color:th.text3,marginBottom:3,letterSpacing:"0.07em"}}>{p.l}</label>
                  <input type="text" value={inp[p.f]||""} onChange={e=>set(p.f,e.target.value)} placeholder="—"
              ...
              <div style={{marginBottom:11}}>
                ...
                <textarea value={inp.planNotu||""} onChange={e=>set("planNotu",e.target.value)}
                  placeholder={lang==="TR"?"Örn: KAKS:1.50 TAKS:0.40 H:9.50 AYRK":"e.g. FAR:1.50 TAKS:0.40 H:9.50 DETACHED"}
                  rows={2}
