      ...
      <div className="pro-content" style={{ flex:1, overflow:"auto", padding:isFullscreen?"24px":"18px" }}>
        ...
        {tab==="aiozet" && (
          <div className="tab-panel tab-panel-aiozet">
            <div style={{fontSize:13,fontWeight:700,color:th.accent,marginBottom:4}}>🤖 {lang==="TR"?"AI YATIRIMCI ÖZETİ":"AI INVESTOR SUMMARY"}</div>
            <div style={{fontSize:10,color:th.text3,marginBottom:16}}>{lang==="TR"?"Projenizin tüm verilerini okuyup profesyonel bir yatırımcı raporu üretir":"Reads all your project data and generates a professional investor report"}</div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              {/* Quick stats for context */}
              <div style={{background:th.card,border:`1px solid ${th.border}`,borderRadius:10,padding:14}}>
                <div style={{fontSize:10,color:th.accent,fontWeight:700,marginBottom:10}}>{lang==="TR"?"ANALIZE GÖNDERILECEK VERİLER":"DATA TO BE ANALYZED"}</div>
                {[
                  ["IRR", r.irrYillik?ƒ.pct(r.irrYillik):"—"],
                  ["WACC", ƒ.pct(r.wacc)],
                  ["Hurdle Rate", ƒ.pct(r.hurdle)],
                  ["Net Kâr", ƒ.tl(r.pazKar)],
                  ["Kâr Marjı", ƒ.pct(r.pazMarji)],
                  ["NBD (NPV)", ƒ.tl(r.nbD)],
                  ["Kâr Erimesi", ƒ.pct(r.karErime)],
                  ["WACC Kararı", r.waccKarar],
                ].map(([k,v],i)=>(
              ...
              <div style={{background:th.card,border:`1px solid ${th.border}`,borderRadius:10,padding:14,display:"flex",flexDirection:"column",gap:12}}>
                <div style={{fontSize:36,textAlign:"center"}}>🤖</div>

                {/* API Key input */}
                <div>
                  <div style={{fontSize:10,color:th.text3,marginBottom:4}}>
                    Anthropic API Key
                    <a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" style={{color:th.accent,marginLeft:6,fontSize:9}}>→ Ücretsiz al</a>
                  </div>
                  <input type="password" value={aiApiKey}
                    onChange={e=>{ setAiApiKey(e.target.value); try{localStorage.setItem("ai_key",e.target.value)}catch{} }}
                    placeholder="sk-ant-..."
                    style={{background:th.input,border:`1px solid ${aiApiKey?th.green+"60":th.accent}40`,color:th.inputText||th.accent,borderRadius:6,padding:"7px 10px",fontSize:11,fontFamily:"monospace",width:"100%",outline:"none"}} />
                  {!aiApiKey && <div style={{fontSize:9,color:"#f59e0b",marginTop:3}}>⚠️ {lang==="TR"?"API key gerekli — console.anthropic.com'dan ücretsiz alın":"API key required — get free at console.anthropic.com"}</div>}
                </div>

                <div style={{fontSize:11,color:th.text2,textAlign:"center",lineHeight:1.6}}>
                  {lang==="TR"?"Projenizin verilerini okuyup profesyonel Uzman Görüşü üretir":"Reads your project data and generates a professional Expert Opinion"}
                </div>

                <button
                  onClick={async ()=>{
                    if (!aiApiKey) { setAiText(lang==="TR"?"⚠️ Lütfen Anthropic API key girin. console.anthropic.com adresinden ücretsiz alabilirsiniz.":"⚠️ Please enter your Anthropic API key. Get one free at console.anthropic.com"); return; }
                    setAiLoading(true); setAiText("");
                    const prompt = lang==="TR"
                      ? `Sen deneyimli bir gayrimenkul yatırım analistissin. Aşağıdaki proje verilerini analiz et ve profesyonel bir "Uzman Görüşü" yaz (200-300 kelime, Türkçe):

Proje: ${[inp.il,inp.ilce,inp.mahalle].filter(Boolean).join("/")||"Belirtilmemiş"} | Ada: ${inp.adaNo||"—"} Parsel: ${inp.parselNo||"—"}
Arsa: ${inp.arsaAlan}m² | KAKS: ${r.kaks.toFixed(2)} | Kat: ${r.katSayisi} | BB: ${r.bbSayisi} adet
Toplam Alan: ${Math.round(r.toplamSat)}m² | Taban: ${Math.round(r.taban)}m²
Maliyet: ${ƒ.tlF(r.topMal)} | Brüt Satış: ${ƒ.tlF(r.brutSatis)}
Net Kâr: ${ƒ.tlF(r.pazKar)} | Kâr Marjı: ${ƒ.pct(r.pazMarji)}
IRR: ${r.irrYillik?ƒ.pct(r.irrYillik):"Hesaplanamadı"} | WACC: ${ƒ.pct(r.wacc)} | Hurdle: ${ƒ.pct(r.hurdle)}
NBD: ${ƒ.tlF(r.nbD)} | Karar: ${r.waccKarar}
Şerefiye Kârı: ${ƒ.tlF(r.sKar)} | Paz. Gider: ${ƒ.tlF(r.pazGider)}

Şunları kapsa: 1) Güçlü yönler 2) Risk faktörleri 3) IRR vs WACC yorumu 4) Tavsiye (Devam Et / Revize Et / Dur)`
                      : `You are an experienced real estate investment analyst. Write a professional "Expert Opinion" (200-300 words):
Location: ${[inp.il,inp.ilce,inp.mahalle].filter(Boolean).join("/")||"N/A"} | Block: ${inp.adaNo||"—"} Parcel: ${inp.parselNo||"—"}
Plot: ${inp.arsaAlan}m² | FAR: ${r.kaks.toFixed(2)} | Floors: ${r.katSayisi} | Units: ${r.bbSayisi}
Cost: ${ƒ.tlF(r.topMal)} | Sales: ${ƒ.tlF(r.brutSatis)} | Profit: ${ƒ.tlF(r.pazKar)} | Margin: ${ƒ.pct(r.pazMarji)}
IRR: ${r.irrYillik?ƒ.pct(r.irrYillik):"N/A"} | WACC: ${ƒ.pct(r.wacc)} | NPV: ${ƒ.tlF(r.nbD)} | Decision: ${r.waccKarar}
Cover: 1) Strengths 2) Risks 3) IRR vs WACC 4) Recommendation`;
                    ...
                    try {
                      ...
                      if (!res.ok) {
                        ...
                        setAiText(`⚠️ API Hatası: ${err.error?.message||res.status}. ${lang==="TR"?"API key'inizi kontrol edin.":"Check your API key."}`);
                      } else {
                        const data = await res.json();
                        const text = data.content?.map(c=>c.text||"").join("") || (lang==="TR"?"Yanıt alınamadı":"No response");
                        setAiText(text);
                      }
                    } catch(e) {
                      setAiText(lang==="TR"?"⚠️ Bağlantı hatası. İnternet bağlantınızı kontrol edin.":"⚠️ Connection error. Check your internet connection.");
                    }
                    setAiLoading(false);
                  }}
                  disabled={aiLoading}
                  style={{background:aiLoading?th.bg3:aiApiKey?`linear-gradient(135deg,${th.accent},#e8c060)`:th.bg3,border:"none",color:aiLoading||!aiApiKey?th.text3:"#070a0f",borderRadius:10,padding:"12px 28px",cursor:aiLoading||!aiApiKey?"default":"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",transition:"all 0.2s",width:"100%"}}>
                  {aiLoading?(lang==="TR"?"🤖 Analiz ediliyor...":"🤖 Analyzing..."):(lang==="TR"?"🚀 AI Analizi Başlat":"🚀 Generate AI Analysis")}
                </button>
              </div>
            </div>
            ...
            {/* AI Output */}
            {aiLoading && (
              <div style={{background:th.card,border:`1px solid ${th.accent}40`,borderRadius:10,padding:24,textAlign:"center"}}>
                <div style={{fontSize:24,marginBottom:8,animation:"pulse 1s infinite"}}>🤖</div>
                <div style={{fontSize:12,color:th.text2}}>{lang==="TR"?"Claude AI projenizi analiz ediyor...":"Claude AI is analyzing your project..."}</div>
                <div style={{marginTop:10,height:4,background:th.bg,borderRadius:2,overflow:"hidden"}}>
                  <div style={{height:"100%",width:"60%",background:`linear-gradient(90deg,${th.accent},#e8c060)`,borderRadius:2,animation:"slide 1.5s ease-in-out infinite"}} />
                </div>
              </div>
            )}

            {aiText && !aiLoading && (
              <div style={{background:isDark?"linear-gradient(135deg,#0d1a08,#091205)":"linear-gradient(135deg,#f0f8e8,#e8f5d8)",border:`1px solid ${th.green}40`,borderRadius:10,overflow:"hidden"}}>
                <div style={{background:th.green,padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:11,fontWeight:700,color:"#fff"}}>🤖 {lang==="TR"?"UZMAN GÖRÜŞÜ":"EXPERT OPINION"} — Claude AI</span>
                  <button onClick={()=>{
                    const el=document.createElement("textarea"); el.value=aiText;
                    document.body.appendChild(el); el.select(); document.execCommand("copy");
                    document.body.removeChild(el);
                  }} style={{background:"transparent",border:"1px solid rgba(255,255,255,0.4)",color:"#fff",borderRadius:4,padding:"2px 8px",cursor:"pointer",fontSize:9,fontFamily:"inherit"}}>
                    📋 {lang==="TR"?"Kopyala":"Copy"}
                  </button>
                </div>
                <div style={{padding:18,fontSize:13,color:th.text,lineHeight:1.9,whiteSpace:"pre-wrap",fontFamily:"'Palatino Linotype',serif"}}>
                  {aiText}
                </div>
                <div style={{padding:"10px 18px",borderTop:`1px solid ${th.border}`,fontSize:9,color:th.text3,fontStyle:"italic"}}>
                  {lang==="TR"?"Bu analiz yapay zeka tarafından üretilmiştir. Yatırım kararları için lisanslı mali müşavire danışınız.":"This analysis is AI-generated. Consult a licensed financial advisor for investment decisions."}
                </div>
              </div>
            )}
          </div>
