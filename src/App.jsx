...
export default function App() {
  ...
  const [aiText, setAiText] = useState("");          // AI summary text
        ...
        {tab==="aiozet" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              ...
              {/* Generate button */}
              <div style={{background:th.card,border:`1px solid ${th.border}`,borderRadius:10,padding:14,display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",gap:12}}>
                <div style={{fontSize:36}}>🤖</div>
                <div style={{fontSize:12,color:th.text2,textAlign:"center",lineHeight:1.6}}>
                  {lang==="TR"
                    ?"Claude AI, projenizin tüm finansal verilerini okuyarak Türkçe uzman görüşü üretir"
                    :"Claude AI reads all your project financial data and generates an expert opinion"}
                </div>
                <button
                  onClick={async ()=>{
...
IRR (Yıllık): ${r.irrYillik?ƒ.pct(r.irrYillik):"Hesaplanamadı"} | WACC: ${ƒ.pct(r.wacc)} | Hurdle: ${ƒ.pct(r.hurdle)}
NBD: ${ƒ.tlF(r.nbD)} | WACC Kararı: ${r.waccKarar}
Kâr Erimesi (Eskalasyon): ${ƒ.pct(r.karErime)}
Şerefiye Kârı: ${ƒ.tlF(r.sKar)} | Pazarlama Gideri: ${ƒ.tlF(r.pazGider)}

Şunları kapsa: 1) Projenin güçlü yönleri 2) Risk faktörleri 3) IRR vs WACC yorumu 4) Tavsiye (Devam Et / Revize Et / Dur)`
                      : `You are an experienced real estate investment analyst. Analyze the following project data and write a professional "Expert Opinion" (200-300 words, English):

Location: ${[inp.il,inp.ilce,inp.mahalle].filter(Boolean).join("/")||"Not specified"}
Plot: ${inp.arsaAlan}m² | FAR: ${r.kaks.toFixed(2)} | Floors: ${r.katSayisi} | Units: ${r.bbSayisi}
Total Area: ${Math.round(r.toplamSat)}m² | Cost: ${ƒ.tlF(r.topMal)}
Net Profit: ${ƒ.tlF(r.pazKar)} | Margin: ${ƒ.pct(r.pazMarji)}
IRR: ${r.irrYillik?ƒ.pct(r.irrYillik):"N/A"} | WACC: ${ƒ.pct(r.wacc)} | Hurdle: ${ƒ.pct(r.hurdle)}
NPV: ${ƒ.tlF(r.nbD)} | Decision: ${r.waccKarar}

Cover: 1) Strengths 2) Risk factors 3) IRR vs WACC interpretation 4) Recommendation (Proceed/Revise/Stop)`;
                    ...
                    try {
                      ...
                      const text = data.content?.map(c=>c.text||"").join("") || "API yanıt vermedi";
                      setAiText(text);
                    } catch(e) {
                      setAiText(lang==="TR"?"Bağlantı hatası. Lütfen tekrar deneyin.":"Connection error. Please try again.");
                    }
                    setAiLoading(false);
                  }}
                  disabled={aiLoading}
                  style={{background:aiLoading?th.bg3:`linear-gradient(135deg,${th.accent},#e8c060)`,border:"none",color:aiLoading?th.text3:"#070a0f",borderRadius:10,padding:"12px 28px",cursor:aiLoading?"default":"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",transition:"all 0.2s",width:"100%"}}>
                  {aiLoading
                    ? (lang==="TR"?"🤖 Analiz ediliyor...":"🤖 Analyzing...")
                    : (lang==="TR"?"🚀 AI Analizi Başlat":"🚀 Generate AI Analysis")}
                </button>
              </div>
            </div>
